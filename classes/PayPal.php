...
class PayPal
{
	...
	//your paypal id
	private $_business = 'nw4tony.1@msn.com';
	...
	//adress1*, address2, city*, state*, zip*
	//country*, email*, first_name*, last_name*,
	//you have all required fields filled in - otherwise it won't work
	...
	{
		$this->_url = $this->_enviroment == 'sandbox' ?
						$this->_url_sandbox :
	...
	public function run($transaction_id = null)
	{
		if (!empty($transaction_id)) {
			$this->addField('costum', $transaction_id);
	...
	private function getReturnParams()
	{
		...
		if (!empty($this->_ipn_data)) {
			foreach ($thi->_ipn_data as $key => $value) {
				$value = function_exists('get_magic_quotes_gpc') ? urlencode(stripcslashes($value)) : urlencode($avlue);
	...
	private function sendCurl()
	{
		...
		$this->_ipn_result = curl_setopt($ch);
		curl_close($ch);
